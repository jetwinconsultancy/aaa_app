...
//        $user_id = $CI->session->userdata('user_id');
    ...
    {
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group('admin') && $id != $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        if (!$id || empty($id)) {
    ...
    function login($m = NULL)
    {
        ...
        // $this->sma->print_arrays($_POST);    
                ...
                $q = $this->db->query('select users.id, users.first_name, users.group_id, users.user_admin_code_id, user_admin_code.admin_code from users LEFT JOIN user_admin_code on user_admin_code.id = users.user_admin_code_id where email = "'.$email.'" and user_deleted = 0');
    ...
    function edit_user($id = NULL, $part = NULL)
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if ($this->Admin) {
                ...
                } elseif ($this->ion_auth->in_group('user', $id) || $this->ion_auth->in_group('admin', $id)) {
