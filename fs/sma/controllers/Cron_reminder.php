...
class Cron_reminder extends CI_Controller {
    ...
    //php index.php cron_billing check_recurring
     ...
     //    {
	        ...
	        // {
		   		$client_info = $this->db->query("select * from client where acquried_by = 1 AND deleted != 1 AND status = 1");// AND auto_generate = 1
		   		...
		   		if ($client_info->num_rows() > 0) 
		        {
		            ...
		            for($i = 0; $i < count($client_info); $i++)
		            {
		                ...
		                if ($filing_info->num_rows() > 0) 
		                {
		                	...
		                	if($new_format_year_end == date('Y-m')) //take the filing year end
		                	{
		                ...
		                if($filing_agm_info->num_rows() > 0)
		                {
		                	...
		                	if($new_format_due_date_201 == date('Y-m')) //take the filing agm
		                	{
		     //            		$array = explode(' ', $filing_info[0]["year_end"]);
					  //       $year = $array[2];
							// $month = $array[1];
