...
class Companies_model extends CI_Model
{
    ...
    public function getAllSalesmanCompanies()
    {
        $q = $this->db->get_where('companies', array('group_name' => 'biller'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getMaxCompanies($firstchar)
    {
		// echo "select COALESCE(c_char,0) as nomor from sma_companies where group_name='supplier' and ucase(companies_code)=ucase('".$firstchar."')";
        $q = $this->db->query("select max(COALESCE(c_char,0)) as nomor from sma_companies where group_name='supplier' and ucase(companies_code)=ucase('".$firstchar."')");
        if ($q->num_rows() > 0) {
            // foreach (($q->result()) as $row) {
                // $data[] = $row;
            // }
    ...
    public function getMaxCustCompanies($firstchar)
    {
		// echo "select COALESCE(c_char,0) as nomor from sma_companies where group_name='supplier' and ucase(companies_code)=ucase('".$firstchar."')";
        $q = $this->db->query("select max(COALESCE(c_char,0)) as nomor from sma_companies where group_name='customer' and ucase(companies_code)=ucase('".$firstchar."')");
        if ($q->num_rows() > 0) {
            // foreach (($q->result()) as $row) {
                // $data[] = $row;
            // }
    ...
    public function getAllSupplierCompanies1()
    {
    ...
    {
        $q = $this->db->query("select * from supplier where id=".$id);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getSalesmanSuggestions($term, $limit = 10)
    {
        $this->db->select("id, company as text");
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR company LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('companies', array('group_name' => 'salesman'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        if ($this->getSalesmanSales($id)) {
            return false;
        }
        if ($this->db->delete('companies', array('id' => $id, 'group_name' => 'salesman'))) {
            return true;
        }
        return FALSE;
    }

    public function getCustomerSuggestions1($term, $limit = 10)
    {
    ...
    {
        $this->db->query("select id, nama as text from customer where (id LIKE '%" . $term . "%' OR nama LIKE '%" . $term . "%' OR alamat LIKE '%" . $term . "%' OR notelp LIKE '%" . $term . "%' OR catatan LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('companies', array('group_name' => 'customer'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    {
        $q =$this->db->query ("select id, nama as text from supplier where (id LIKE '%" . $term . "%' OR nama LIKE '%" . $term . "%' OR alamat LIKE '%" . $term . "%' OR notelp LIKE '%" . $term . "%' OR catatan LIKE '%" . $term . "%') ");
    ...
    public function getSupplierSuggestions1($term, $limit = 10)
    {
        $this->db->select("id, CONCAT(company, ' (', name, ')') as text, payment_term", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR company LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('companies', array('group_name' => 'supplier'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
