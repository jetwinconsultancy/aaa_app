...
class Products_model extends CI_Model
{
    ...
    { 
        $q = $this->db->query('select * from bahan_utama');
        if ($q->num_rows() > 0) {
    ...
    { 
        $q = $this->db->query('select * from bahan_kaos');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getAllPola()
    { 
        $q = $this->db->get('p_pola');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNameProducts($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select token as id,token as name from sma_p_token WHERE token LIKE '%%%".$name."%%' ORDER BY token asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNameKategoriBahan($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select kategori as id,kategori as name from kategori_bahan WHERE kategori LIKE '%%%".$name."%%' ORDER BY kategori asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNameJenisBahan($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select jenis as id,jenis as name from jenis_bahan WHERE jenis LIKE '%%%".$name."%%' ORDER BY jenis asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNameWarnaBahan($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select warna as id,warna as name from warna_bahan WHERE warna LIKE '%%%".$name."%%' ORDER BY warna asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNameMotifBahan($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select motif as id,motif as name from motif_bahan WHERE motif LIKE '%%%".$name."%%' ORDER BY motif asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getNamePrintingBahan($name)
    {
			$rw['id'] =$name;
			$rw['name'] =$name;
			$data[] = $rw;
        $q = $this->db->query("select printing as id,printing as name from printing_bahan WHERE printing LIKE '%%%".$name."%%' ORDER BY printing asc LIMIT 10");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function addToken($name)
    {
		if ($name) {
			foreach ($name as $item) {
				$this->db->query("insert into sma_p_token(token) select '".$item."' from sma_p_token  where NOT EXISTS (select token from sma_p_token where token='".$item."') limit 1 ");
			}
		}
    }
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)
    {
		...
		// print_r( $this->db->_error_message(););
			...
			// $this->sma->print_arrays($this->db->insert('products',$data);
    ...
    public function addProduct2($data, $items, $warehouse_qty, $product_attributes, $photos, $data_purchase, $product_purchase)
    {
		// $this->db->insert('products', $data);
		// print_r( $this->db->_error_message(););
			// $this->sma->print_arrays($data);
        ...
        if ($this->db->insert('products', $data)) {
			...
			{
				$data_purchase[$i]['reference_no'] = $this->site->getReference('po');
				if ($this->db->insert('purchases', $data_purchase[$i])) {
					$purchase_id = $this->db->insert_id();
					if ($this->site->getReference('po') == $data_purchase[$i]['reference_no']) {
						$this->site->updateReference('po');
					}
					$item = $product_purchase[$i];
					{
						$item['product_id'] = $product_id;
	...
	public function syncAdjustment1($data = array())
    {
        if(! empty($data)) {
            if($data['type'] == 'subtraction') {
    ...
    public function reverseAdjustment1($id)
    {
        ...
        if($adjustment) {
            if($adjustment->type == 'subtraction') {                
					...
					$this->db->where('id',   $adjustment->product_id);
					$this->db->update('products');
					$this->db->set('quantity', 'quantity-'.$adjustment->quantity, FALSE);
					$this->db->where('id',  $adjustment->product_id);
					$this->db->update('warehouses_products');
