...
class Purchases_model extends CI_Model
{
    ...
    public function getProductNames($term,$supplier_id, $limit = 5)
    {
        $q = $this->db->query("select concat('B',A.id) as id,concat('Bahan ',kategori,',',jenis,',',motif,',',warna,',',printing,',',nama) as name, B.kode_supplier as code2, C.satuan as unit, B.harga from bahan_utama A,sup_bahan B,satuan C where C.id = A.satuan and B.id_barang = A.id and B.id_sup=".$supplier_id." and kategori like '%".$term."%'");
        // $q = $this->db->query("select * from bahan_utama A,sup_bahan B where ");
        if ($q->num_rows() > 0) {
			...
			$q = $this->db->query("select concat('A',A.id) as id,concat(nama,',',warna) as name, B.kode_supplier as code2, C.satuan as unit, B.harga from bahan_kaos A,sup_kaos B,satuan C where C.id = A.satuan and B.id_barang = A.id and B.id_sup=".$supplier_id." and A.nama like '%".$term."%' and A.warna like '%".$term."%'");
			if ($q->num_rows() > 0) {
				foreach (($q->result()) as $row) {
					$data[] = $row;
				}
    ...
    {
        $q = $this->db->query("select * from bahan_kaos where id=".$code);
        // $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getProductssByCode($code)
    {
