...
function printElement(elem, append, delimiter) {
	...
	function update_harga_terakhir(data){
		data = data.replace('[',"");
		data = data.replace(']',"");
		data_harga = data.split(",");
		slitems = JSON.parse(localStorage.getItem('slitems'));
		produc_id = new Array();
		lasts_price = new Array();	
		urutan =0;
		$.each(slitems, function () {
			var item = this;
			var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
			if (typeof(data_harga[urutan]) === "undefined")
			{
				
			}else {
				item.row.last_price =data_harga[urutan];
			}
			slitems[item_id] = item;
		// data_harga.forEach(function(entry) {
			// $(".bariske_"+urutan).find(".slast_price").html(entry);
			urutan++;
		});
		localStorage.setItem('slitems', JSON.stringify(slitems));
		loadItems();
	}
	function cek_harga_customer(customer){
		slitems = JSON.parse(localStorage.getItem('slitems'));
		produc_id = new Array();
		lasts_price = new Array();
		banyak_produk = 0;
		$.each(slitems, function () {
			...
			var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
			slitems[item_id] = item;
			produc_id[banyak_produk] = item.row.id;
			if (typeof item.row.last_price === 'undefined') { lasts_price[banyak_produk] = 0;} else {lasts_price[banyak_produk] = item.row.last_price;}
			banyak_produk++;
		});
				// alert(JSON.stringify(produc_id));
		$.ajax({
			type: 'get',
			url: site.base_url+"sales/customer_harga",
			data: {
				//term: request.term,
				data_id_produk: produc_id,
				customer_id: customer
			},
			success: function (data) {
				update_harga_terakhir(data);
				// response(data);
			}
		});
		// alert(custom	er);
...
if (localStorage.getItem('slitems')) {
	if($add_sale_price) loadItems1(); else loadItems();
}
...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
	 ...
	 * Edit Row Quantity Method 
	 ...
	 $(document).on("focus", '.rquantity', function () {
		...
		if(event.which == 38)
		{
			...
			slitems[item_id].row.qty = new_qty;
			localStorage.setItem('slitems', JSON.stringify(slitems));
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),38,'rquantity');
		}
		if(event.which == 40)
		{
			var row = $(this).closest('tr');
			...
			slitems[item_id].row.qty = new_qty;
			localStorage.setItem('slitems', JSON.stringify(slitems));
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),40,'rquantity');
		}
		if(event.which == 13)
		{
			var row = $(this).closest('tr');
			...
			slitems[item_id].row.qty = new_qty;
			localStorage.setItem('slitems', JSON.stringify(slitems));
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),13,'rquantity');
		}
	});
	var old_row_unit;
	 $(document).on("focus", '.runit', function () {
		...
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),38,'runit');
		}
		if(event.which == 40)
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),40,'runit');
		}
		if(event.which == 13)
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),13,'runit');
		}
	});
	...
	var old_row_unit;
	 $(document).on("focus", '.rdiscount', function () {
		// old_row_qty = $(this).val();
	}).on("keydown",'.rdiscount', function (event) {
		// alert(event.which);
		if(event.which == 38)
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),38,'rdiscount');
		}
		if(event.which == 40)
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),40,'rdiscount');
		}
		if(event.which == 13)
		{
			var row = $(this).closest('tr');
			if($add_sale_price) loadItems1(); else loadItems(row.attr('data-item-no'),13,'rdiscount');
		}
	});
	 ...
	 * Edit Row Price Method 
	 ...
	 $(document).on("focus", '.rprice', function () {
		...
		slitems = JSON.parse(localStorage.getItem('slitems'));
		// alert(slitems[item_id].row.price);
		loadItems();
	});
	 ...
	 * Edit Row Price Method 
	 -------------------------- */
	 var old_price;
	 $(document).on("focus", '.realuprice', function () {
		old_price = $(this).val();
		$(this).select();
	}).on("change", '.realuprice', function () {
		$nilai = $(this).val();
		var row = $(this).closest('tr');
		if (!is_numeric($(this).val())) {
			// $(this).val(old_price);
	...
	}).on("keydown",'.realuprice', function (event) {
		// alert(event.which);
		if(event.which == 38)
		{
			var row = $(this).closest('tr');
			...
			slitems = JSON.parse(localStorage.getItem('slitems'));
			loadItems(row.attr('data-item-no'),38,'realuprice');
		}
		if(event.which == 40)
		{
			var row = $(this).closest('tr');
			...
			slitems = JSON.parse(localStorage.getItem('slitems'));
			loadItems(row.attr('data-item-no'),40,'realuprice');
		}
		if(event.which == 13)
		{
			var row = $(this).closest('tr');
			...
			slitems = JSON.parse(localStorage.getItem('slitems'));
			loadItems(row.attr('data-item-no'),13,'realuprice');
		}
	});
...
function loadItems(lokasi_baris,tombol,komponennya) {
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			var last_price = 0;
			if (typeof item.row.last_price === 'undefined') {} else {last_price = item.row.last_price;}
			// alert(item.row.lastprice);
			var unit = item.row.unit;
			var unit1 = item.row.unit1;
			var unit2 = item.row.unit2;
			...
			tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+'</span> <i class="pull-right fa fa-edit tip pointer edit hidden" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
			...
			tr_html += '</select></td>';
			tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '">';
			tr_html += '<span class="slast_price">'+formatMoney(last_price,0)+'</span></td>';
			tr_html += '<td class="text-right">';
			tr_html += '<input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '">';
			tr_html += '<input class="realuprice form-control text-right" name="real_unit_price[]" style="width:120px;" type="text" value="' + item.row.real_unit_price + '">';
			tr_html += '<span class="text-right hidden sprice" id="sprice_' + row_no + '">' + formatMoney(item_price) + '</span></td>';
			
			// if (site.settings.product_discount == 1) {
				// tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="text" id="discount_' + row_no + '" value="' + item_ds + '">';
				// tr_html += '<span class="text-right hidden sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(0 - (item_discount * item_qty)) + '</span></td>';
			// }
			if (site.settings.tax1 == 1) {
			...
			// alert(item.row.qty_kirim);
			if (item.row.qty_kirim >0)
			{
...
function next_element(lokasi_baris,tombol,komponennya) {
	...
	if (tombol == 13)	
	{
		...
		if (komponennya == "realuprice") $("#add_item").focus();
		...
		if (komponennya == "runit") $(".bariske_"+lokasi_baris).find('.realuprice').focus();
	}
}
function loadItems1() {
	...
	if (localStorage.getItem('slitems')) {
		...
		$("#sUpdate_price_Table tbody").empty();
		slitems = JSON.parse(localStorage.getItem('slitems'));
		$('#add_sale, #edit_sale').attr('disabled', false);
		$.each(slitems, function () {
			var item = this;
			...
			tr_html += '<td class="text-right"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(item.row.cost) + '</span></td>';
			tr_html += '<td class="text-right"><input class="  rprice" name="net_price[]" type="text" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(item_price) + '</span></td>';
			tr_html += '<td><input class="form-control text-center sprice" name="saleprice[]" type="text" value="'+item.row.qty_kirim+"-" + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="saleprice_' + row_no + '" onClick="this.select();"></td>';
			alert(item.row.qty_kirim);
			if (item.row.qty_kirim >0)
			{
				tr_html += '<td class="text-center"></td>';
			...
			newTr.html(tr_html);
			newTr.prependTo("#sUpdate_price_Table");
			// total += formatDecimal(item_price * item_qty);
			total += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
			count += parseFloat(item_qty);
			an++;
		...
		//$('#tds').text('('+formatMoney(product_discount)+'+'+formatMoney(order_discount)+')'+formatMoney(total_discount));
		if (an > site.settings.bc_fix && site.settings.bc_fix != 0) {
			$("html, body").animate({scrollTop: $('#sUpdate_price_Table').offset().top - 150}, 500);
			$(window).scrollTop($(window).scrollTop() + 1);
		}
		if (count > 1) {
			$('#slcustomer').select2("readonly", true);
...
function add_invoice_item2(item) {
