...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
	 ...
	 * Edit Row Quantity Method 
	 ...
	 $(document).on("focus", '.rquantity', function () {
		...
		localStorage.setItem('slitems', JSON.stringify(slitems));
		if($add_sale_price) loadItems1(); else loadItems();
	}).on("keyup",'.rquantity', function (event) {
		// alert(event.which);
		if(event.which == 38)
		{
			$(this).closest('tr').prevAll(':has(.rquantity):first').find('.rquantity').focus();
		}
		if(event.which == 40)
		{
			$(this).closest('tr').nextAll(':has(.rquantity):first').find('.rquantity').focus();
		}
	});
	 ...
	 * Edit Row Price Method 
	 ...
	 $(document).on("focus", '.realuprice', function () {
		...
		slitems = JSON.parse(localStorage.getItem('slitems'));
		loadItems();
	}).on("keyup",'.realuprice', function (event) {
		// alert(event.which);
		if(event.which == 38)
		{
			$(this).closest('tr').prevAll(':has(.realuprice):first').find('.realuprice').focus();
		}
		if(event.which == 40 || event.which == 13)
		{
			...
			slitems = JSON.parse(localStorage.getItem('slitems'));
			loadItems();
			$(this).closest('tr').nextAll(':has(.realuprice):first').find('.realuprice').focus();
		}
	});
...
function loadItems() {
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			tr_html += '<td><select class="form-control text-center " name="unit[]">';
