												...
												<div class="tab-content">
													<div id="w2-account" class="tab-pane active">
														...
														<div class="form-group">
															...
															<div class="col-sm-4">
																<input type="text" class="form-control input-sm" maxlength="10"  name="uen" value="<?=$client->uen?>" >
															</div>
														</div>
			...
			<table class="table table-bordered table-striped table-condensed mb-none" >
				<thead>
					<tr>
						<th>ID</th>
						<td>
						<input type="text" class="form-control" name="id" value="" id="gid_add_officer"/></td>
					</tr>
					<tr>
						<th>Name</th>
...
$('form#Save_Officer').submit(function(e) {
    ...
    $.ajax({
		...
		$("#gid_add_officer").on('change',function(){
			$.ajax({
				type: "GET",
				url: "masterclient/get_officer/"+$(this).val(),
				// data: {"gid":$(this).val()}, // <--- THIS IS THE CHANGE
				// dataType: "json",
				success: function(data){
					console.log(data);
					$b = JSON.parse(data);
					$("#add_officer_nama").val($b['nama']);
