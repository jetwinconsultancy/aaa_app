...
$(document).ready(function() {
    ...
    $('body').on('click', '#group_inv', function(e) {
        e.preventDefault();
        $('#form_action').val($(this).attr('data-action'));
        $('#action-form-submit').trigger('click');
    });
...
$(document).ready(function() {
    ...
    $('body').on('click', '.repacking_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'inventory/modal_view/' + $(this).parent('.repacking_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
...
$(".btn-simpan").on('click',function(){
	form = $(this).closest('form').ajaxSubmit();
	// $.ajax( {
      // type: "POST",
