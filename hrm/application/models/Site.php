...
class Site extends CI_Model
{
    ...
    public function getProductByID_X($id,$code) {
		// if ($code == 'A')
		// {
			// $q = $this->db->query("select concat('A',A.id) as id,concat(nama,',',warna) as name, B.kode_supplier as code2, C.satuan as unit, B.harga from bahan_kaos A,sup_kaos B,satuan C where C.id = A.satuan and B.id_barang = A.id and A.id=".$id) ;
		// } else 
		// {
			// $q = $this->db->query("select concat('B',A.id) as id,concat('Bahan ',kategori,',',jenis,',',motif,',',warna,',',printing,',',nama) as name, B.kode_supplier as code2, C.satuan as unit, B.harga from bahan_utama A,sup_bahan B,satuan C where C.id = A.satuan and B.id_barang = A.id and A.id=".$id) ;
		// }
        // if ($q->num_rows() > 0) {
            // return $q->row();
        // }
    ...
    public function getUserType($user_id = false) {
        if (!$user_id) {
            $user_id = $this->session->userdata('user_id');
        }
        $user_type_id = $this->getUserTypeID($user_id);
        $q = $this->db->get_where('user_type', array('id' => $user_type_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getUserTypeID($user_id = false) {
        $user = $this->getUser($user_id);
    ...
    public function syncSalePayments_group($id) {
        $sale = $this->getSaleByID($id);
        $payments = $this->getSalePayments($id);
        $paid = 0;
        foreach ($payments as $payment) {
    ...
    public function checkReceived($purchase_id)
    {
        ...
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
				// print_r($row);
				$jlh_barang = $this->getQtyItemRecieved($purchase_id,$row->product_id);
				// echo "<br/>";
				// print_r($jlh_barang);
				// echo $jlh_barang[0]->quantity.'-'.$row->quantity;
				if (isset($jlh_barang[0]->quantity))
				{
					if ($jlh_barang[0]->quantity < $row->quantity)
					{
						$item_full = false;
					}
				} else {
	...
	public function syncProductStefQTY($product_id){
		$purchase_qty = 0;
		$confirm_delivered_qty = 0;
		$progress_deliver_qty = 0;
