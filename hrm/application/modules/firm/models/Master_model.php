...
class Master_model extends CI_Model
{
    ...
    public function get_all_firm_info($search = null){
        ...
        if($search == null)
        {
            $this->db->select('firm.*, firm_telephone.telephone, firm_fax.fax, firm_email.email, user_firm.user_id, user_firm.default_company, user_firm.in_use')
                ->from('firm')
                ->join('user_firm', 'user_firm.firm_id = firm.id', 'left')
        ...
        else
        {
            $this->db->select('firm.*, firm_telephone.telephone, firm_fax.fax, firm_email.email, user_firm.user_id, user_firm.default_company, user_firm.in_use');
            $this->db->from('firm');
            $this->db->join('user_firm', 'user_firm.firm_id = firm.id', 'left');
	...
	public function get_all_person($term = null,$type = null,$identification_no = null){
        ...
        if($type == "all")
        {
            if ($term)
            {
               // $p_where = ' where ';
                ...
                $p = $this->db->query("select field_type, identification_no, name, date_of_birth, officer_mobile_no.mobile_no as local_mobile, officer_email.email from officer left join officer_mobile_no on officer_mobile_no.officer_id = officer.id and officer_mobile_no.primary_mobile_no = 1 left join officer_email on officer_email.officer_id = officer.id and officer_email.primary_email = 1 where ".$p_where."user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by identification_no");

                $r = $this->db->query("select field_type, register_no, company_name, officer_company_phone_number.phone_number as company_phone_number, officer_company_email.email as company_email from officer_company left join officer_company_email on officer_company_email.officer_company_id = officer_company.id and officer_company_email.primary_email = 1 left join officer_company_phone_number on officer_company_phone_number.officer_company_id = officer_company.id and officer_company_phone_number.primary_phone_number = 1 where ".$r_where."user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by register_no");
                ...
                if(count($all) == 0)
                {
                    ...
                    {*/
                        ...
                        $p = $this->db->query("select field_type, identification_no, name, date_of_birth, officer_mobile_no.mobile_no as local_mobile, officer_email.email from officer left join officer_mobile_no on officer_mobile_no.officer_id = officer.id and officer_mobile_no.primary_mobile_no = 1 left join officer_email on officer_email.officer_id = officer.id and officer_email.primary_email = 1 where ".$where_p."user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by identification_no");

                        $where_r = ' register_no like "%'.$term.'%" AND ';
                        $r = $this->db->query("select select field_type, register_no, company_name, officer_company_phone_number.phone_number as company_phone_number, officer_company_email.email as company_email from officer_company left join officer_company_email on officer_company_email.officer_company_id = officer_company.id and officer_company_email.primary_email = 1 left join officer_company_phone_number on officer_company_phone_number.officer_company_id = officer_company.id and officer_company_phone_number.primary_phone_number = 1 where".$where_r."user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by register_no");
        ...
        elseif($type == "individual")
        {
    		if ($term)
    		{
                ...
                $q = $this->db->query("select field_type, identification_no, name, date_of_birth, officer_mobile_no.mobile_no as local_mobile, officer_email.email from officer left join officer_mobile_no on officer_mobile_no.officer_id = officer.id and officer_mobile_no.primary_mobile_no = 1 left join officer_email on officer_email.officer_id = officer.id and officer_email.primary_email = 1 where ".$where."AND user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by identification_no");
        ...
        elseif($type == "company")
        {
            if ($term)
            {
                ...
                $q = $this->db->query("select field_type, register_no, company_name, officer_company_phone_number.phone_number as company_phone_number, officer_company_email.email as company_email from officer_company left join officer_company_email on officer_company_email.officer_company_id = officer_company.id and officer_company_email.primary_email = 1 left join officer_company_phone_number on officer_company_phone_number.officer_company_id = officer_company.id and officer_company_phone_number.primary_phone_number = 1 where ".$where."AND user_admin_code_id = '".$this->session->userdata("user_admin_code_id")."' group by register_no");
    ...
    {   
        /*if ($company_code)
        {*/
            $q = $this->db->query('select client_billing_info.*, billing_info_service.service as service_name, billing_info_frequency.frequency as frequency_name from client_billing_info left join billing_info_service on client_billing_info.service = billing_info_service.id left join billing_info_frequency on client_billing_info.frequency = billing_info_frequency.id where company_code ="'.$company_code.'" order by client_billing_info_id');
