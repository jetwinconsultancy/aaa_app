...
class Firm extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $bc = array(array('link' => '#', 'page' => 'Our Firm'));
        $meta = array('page_title' => 'Our Firm', 'bc' => $bc, 'page_name' => 'Our Firm');
    ...
    {
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $bc = array(array('link' => '#', 'page' => 'Create Our Firm'));
        $meta = array('page_title' => 'Create Our Firm', 'bc' => $bc, 'page_name' => 'Create Our Firm');
        ...
        $this->mybreadcrumb->add('Our Firm', base_url('firm'));
    ...
    public function edit($id, $tab = null)
    {
        ...
        //$this->data['template'] = $this->master_model->get_all_template();
                ...
                if(!isset($_POST["firm_id"]))
                {
                    ...
                    else
                    {
                        ...
                        for($k = 0; $k < count($billing_id); $k++)
                        {
                            ...
                            $billing_service_info = $billing_service_info->result_array();
                            if(count($billing_service_info) > 0)
                            {
                                $new_amount = (float)$billing_service_info[0]["totalAmount"];
                                $new_outstanding = (float)$billing_service_info[0]["totalAmount"] - ((float)$billing_id[$k]['amount'] - (float)$billing_id[$k]['outstanding']);
        ...
        {*/
           ...
           //$filesCount = count($_FILES['uploadimages']['name']);
            ...
            //{   
                ...
                //$uploadPath = $_SERVER['SERVER_NAME'].'/secretary/uploads/logo';
                //$uploadPath = './uploads/logo';
                $uploadPath = '../secretary/uploads/logo';
                ...
                if($this->upload->do_upload('logo'))
                {
                    ...
                    //unlink($_SERVER['SERVER_NAME']."/secretary/uploads/logo/".$file_info[0]["file_name"]);
