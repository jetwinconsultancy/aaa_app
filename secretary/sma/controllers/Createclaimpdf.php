...
class Createclaimpdf extends MY_Controller 
{
	...
	// public function delete_claim_pdf()
 //    {
 ...
 //    }
	...
	public function create_claim_pdf()
	{
		if($_POST["tab"] == "claim")
		{
			$pv_id = $_POST["pv_id"];
			...
			// {
				...
				// {
					$q = $this->db->query("select claim.firm_id, claim.id, claim.user_id, claim.claim_date, claim.claim_no, claim.currency_id, claim.amount, claim.rate, claim.user_name, claim.postal_code, claim.street_name, claim.building_name, claim.unit_no1, claim.unit_no2, currency.currency from claim left join currency on currency.id = claim.currency_id where claim.id =".$pv_id."");

			       	$q = $q->result_array();

			       	$p = $this->db->query("select claim.id, claim.cheque_number, claim.user_id, claim.bank_acc_id, claim_service.claim_id as claim_service_claim_id, claim_service.client_name, claim_service.claim_description, claim_service.amount, bank_info.banker, currency.currency, bank_info.account_number, users.first_name, users.last_name from claim left join claim_service on claim_service.claim_id = claim.id left join bank_info on bank_info.id = claim.bank_acc_id left join currency on currency.id = bank_info.currency left join users on users.id = claim.approved_by where claim.id =".$pv_id." ORDER BY claim_service.id");

			       	$p = $p->result_array();

			       	//echo json_encode($p[0]["banker"]);
					...
					$receiver_info = $this->receiver_info("Claim", $gst_number_display, $q);
					...
					{	
						...
						$table_part_content = '<tr style="height: 17px;">
												...
												<p style="text-align: justify;">'. nl2br($p[$j]['claim_description']). '<strong></strong></p>
												</td>
					...
					//echo json_encode(sprintf('%0.2f', $total));
					...
					$obj_pdf->Output($_SERVER['DOCUMENT_ROOT'].$this->systemName.'/pdf/claim/'.$q[0]["claim_no"].'.pdf', 'F');

					chmod($_SERVER['DOCUMENT_ROOT'].$this->systemName.'/pdf/claim/'.$q[0]["claim_no"].'.pdf',0644);

					// output: http://
	    			//$protocol = strtolower(substr($_SERVER["SERVER_PROTOCOL"],0,5))=='https://'?'https://':'http://';
					$protocol = isset($_SERVER["HTTPS"]) ? 'https://' : 'http://';
					array_push($array_link,$protocol . $_SERVER['SERVER_NAME'] .'/'.$this->systemName.'/pdf/claim/'.$q[0]["claim_no"].'.pdf');
					//echo ("123");
				//}
	...
	public function receiver_info($document_type, $gst_number_display, $q)
	{
		...
		{
			$document_date = date('d F Y', strtotime(str_replace('/', '-', $q[0]["claim_date"])));
			$document_num  = $q[0]["claim_no"];
		...
		$receiver_info = '<table style="width: 100%; border-collapse: collapse; font-family: arial, helvetica, sans-serif; font-size: 10pt;" border="0">
						...
						<td style="width: 100%; height: 20px; text-align: left;"><strong><span style="font-size: 10pt; font-family: arial, helvetica, sans-serif;">'.$q[0]["user_name"].'</span></strong></td>
						</tr>
						
						<tr style="height: 20px;">
						<td style="width: 100%; height: 20px; text-align: left;"><span style="font-size: 9pt; font-family: arial, helvetica, sans-serif;">By Hand</span></td>
						</tr>
						</tbody>
						</table>
						</td>
						...
						<td style="width: 93.077%; height: 34px;"><span style="text-align: left; font-size: '. $title_font_size .'; font-family: arial, helvetica, sans-serif;">'. strtoupper("PAYMENT") .'</span></td>
