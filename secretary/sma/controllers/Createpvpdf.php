...
class Createpvpdf extends MY_Controller 
{
 ...
 //    }
	...
	public function create_pv_pdf()
	{
		if($_POST["tab"] == "payment_voucher")
		{
			...
			// {
				...
				// {
					$q = $this->db->query("select payment_voucher.firm_id, payment_voucher.id, payment_voucher.supplier_code, payment_voucher.payment_voucher_date, payment_voucher.payment_voucher_no, payment_voucher.currency_id, payment_voucher.amount, payment_voucher.rate, payment_voucher.vendor_name, payment_voucher.postal_code, payment_voucher.street_name, payment_voucher.building_name, payment_voucher.unit_no1, payment_voucher.unit_no2, payment_voucher.foreign_address1, payment_voucher.foreign_address2, payment_voucher.foreign_address3, vendor_info.address_type, currency.currency from payment_voucher left join currency on currency.id = payment_voucher.currency_id left join vendor_info on vendor_info.supplier_code = payment_voucher.supplier_code where payment_voucher.id =".$pv_id."");

			       	$q = $q->result_array();

			       	$p = $this->db->query("select payment_voucher.id, payment_voucher.bank_acc_id, payment_voucher.cheque_number, payment_voucher.supplier_code, payment_voucher_service.payment_voucher_id as payment_voucher_service_payment_voucher_id, payment_voucher_service.payment_voucher_description, payment_voucher_service.amount, bank_info.banker, bank_info.account_number, currency.currency, users.first_name, users.last_name from payment_voucher left join payment_voucher_service on payment_voucher_service.payment_voucher_id = payment_voucher.id left join bank_info on bank_info.id = payment_voucher.bank_acc_id left join currency on currency.id = bank_info.currency left join users on users.id = payment_voucher.approved_by where payment_voucher.id =".$pv_id." ORDER BY payment_voucher_service.id");
					...
					$receiver_info = $this->receiver_info("Payment", $gst_number_display, $q);
					...
					{	
						...
						$table_part_content = '<tr style="height: 17px;">
												...
												<p style="text-align: justify;">'. nl2br($p[$j]['payment_voucher_description']). '<strong></strong></p>
												</td>
					...
					if($q[0]['rate'] != 1)
					{
						...
						$show_converted_total = '<tr>
								<td style="width: 63%;"><span style="font-size: 8pt;">SGD: '. $this->spell_number_dollar(number_format((float)$converted_total, 2, '.', '')) .'</span></td>
					...
					// 			</tr>
					// 			</tbody>
					// 			</table>$this->spell_number_dollar(
					$content_gst  = '
					...
					$content3 = '<table style="width: 100%; border-collapse: collapse;" border="0">
								<tbody>
								<tr>
								<td style="width: 63%;"><span style="font-size: 8pt;">'. $q[0]["currency"] .': '. $this->spell_number_dollar(number_format((float)$total, 2, '.', '')) .'</span></td>
	...
	public function receiver_info($document_type, $gst_number_display, $q)
	{
		...
		{
			$document_date = date('d F Y', strtotime(str_replace('/', '-', $q[0]["payment_voucher_date"])));
			$document_num  = $q[0]["payment_voucher_no"];
		...
		$receiver_info = '<table style="width: 100%; border-collapse: collapse; font-family: arial, helvetica, sans-serif; font-size: 10pt;" border="0">
						...
						<td style="width: 100%; height: 20px; text-align: left;"><strong><span style="font-size: 10pt; font-family: arial, helvetica, sans-serif;">'.$q[0]["vendor_name"].'</span></strong></td>
						</tr>
