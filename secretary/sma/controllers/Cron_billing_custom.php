...
class Cron_billing_custom extends CI_Controller { 
  ...
  //       if($to_billing_cycle != null)
    ...
    public function resendupdate() {
    	...
    	$q = $this->db->query("select recurring_billing.firm_id, recurring_billing.id, recurring_billing.company_code, recurring_billing.invoice_date, recurring_billing.invoice_no, recurring_billing.currency_id, recurring_billing.amount, recurring_billing.rate, recurring_billing.outstanding, recurring_billing.status, recurring_billing.recurring_status, recurring_billing.billing_period, recurring_billing.recu_invoice_issue_date, client.id as clientid, client.acquried_by, client.company_name, client.postal_code, client.street_name, client.building_name, client.unit_no1, client.unit_no2, client.foreign_add_1, client.foreign_add_2, client.foreign_add_3, client.use_foreign_add_as_billing_add, currency.currency from recurring_billing left join client on client.company_code = recurring_billing.company_code left join currency on currency.id = recurring_billing.currency_id where client.acquried_by = '1' AND client.deleted != '1' AND client.status = '1' AND recurring_billing.status = '0' AND recurring_billing.amount != '0.00' and recurring_billing.id IN('98','356','349','318','88','110','141','338','329','58','76','95','119','343','325','63','81','102','129','361','348','321','70','87','107','138','336','355','312','116','94','118','342','324','62','80','100','128','313','347','320','69','86','106','136','335','354','315','74','93','114','341','326','61','79','125','346','319','68','115','105','135','334','352','316','73','92','113','340','327','60','78','97','122','345','322','67','83','104','134','333','350','317','72','89','112','142','339','328','77','59','96','120','344','323','64','82','103','130')");   
		...
		foreach ($q as $key => $value) {  
			...
			// $b = $this->db->query("select billing.invoice_no,billing.id from billing where company_code='".$value->company_code."' ORDER BY updated_at DESC LIMIT 3")->result();
 
			// foreach ($b as $keys => $values) {    
			// 	$bs = $this->db->query("select billing_service.* from billing_service where billing_service.billing_id='".$values->id."'")->result();
			// 	$b[$keys]->services = $bs;  
			// 	$b[$keys]->q = $value->id;  
			...
			$bs = $this->db->query("select recurring_billing_service.id, recurring_billing_service.period_start_date, recurring_billing_service.period_end_date from recurring_billing_service where billing_id='".$value->id."' AND (period_start_date <> '' OR period_start_date <> NULL) ORDER BY period_start_date DESC")->result();
			echo $bs[0]->period_start_date." - ".$bs[0]->period_end_date." | ".$value->id." | ";
			echo $this->encryption->decrypt($value->company_name);  
    ...
    public function resend() 
   	{
		...
		$number_of_invoice = 0;
   		...
   		$q = $this->db->query("select recurring_billing.firm_id, recurring_billing.id, recurring_billing.company_code, recurring_billing.invoice_date, recurring_billing.invoice_no, recurring_billing.currency_id, recurring_billing.amount, recurring_billing.rate, recurring_billing.outstanding, recurring_billing.status, recurring_billing.recurring_status, recurring_billing.billing_period, recurring_billing.recu_invoice_issue_date, client.acquried_by, client.company_name, client.postal_code, client.street_name, client.building_name, client.unit_no1, client.unit_no2, client.foreign_add_1, client.foreign_add_2, client.foreign_add_3, client.use_foreign_add_as_billing_add, currency.currency from recurring_billing left join client on client.company_code = recurring_billing.company_code left join currency on currency.id = recurring_billing.currency_id where client.acquried_by = '1' AND client.deleted != '1' AND client.status = '1' AND recurring_billing.status = '0' AND recurring_billing.amount != '0.00' and recurring_billing.id IN('98','356','349','318','88','110','141','338','329','58','76','95','119','343','325','63','81','102','129','361','348','321','70','87','107','138','336','355','312','116','94','118','342','324','62','80','100','128','313','347','320','69','86','106','136','335','354','315','74','93','114','341','326','61','79','125','346','319','68','115','105','135','334','352','316','73','92','113','340','327','60','78','97','122','345','322','67','83','104','134','333','350','317','72','89','112','142','339','328','59','77','96','120','344','323','64','82','103','130')");
