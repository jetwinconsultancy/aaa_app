...
class Cron_reminder extends CI_Controller {
  ...
  //  		if($day == 'Sat' || $day == 'Sun'){
		...
		// if(!$weekendDay)
		// {
   				//$firm_id = "9"; //change this firm ID to manual send

		   		$client_info = $this->db->query("select * from client where acquried_by = 1 AND deleted != 1 AND status = 1");// AND auto_generate = 1  // AND firm_id = '".$firm_id."'
		   		...
		   		if ($client_info->num_rows() > 0) 
		        {
		            ...
		            for($i = 0; $i < count($client_info); $i++)
		            {
		                ...
		                if ($filing_info->num_rows() > 0) 
		                {
		                	...
		                	//echo json_encode($filing_info[0]);
					        $array = explode(' ', $filing_info[0]["year_end"]);
					        $year = $array[2];
							$month = $array[1];
							$day = $array[0];

						    $new_format_year_end = date('Y-m-d', strtotime('1'.'-'.$month.'-'.(int)$year)); //change this date to manual send

						    //$new_format_year_end = date('Y-m', strtotime($month.'-'.(int)$year));

		                	//if($new_format_year_end == '1980-02') //take the filing year end 2020
		                	if($new_format_year_end == date('Y-m-d')) //take the filing year end
		                	{
			                	...
			                	{	
			                		...
			                		for($v = 0; $v < 1; $v++) //count($check_selected_reminder)
	        						{
				                		...
				                		if ($client_contact_info->num_rows() > 0) 
					                	{
							                ...
							                if ($select_contact_persons->num_rows() > 0) 
			                				{
			                					...
			                					for($t = 0; $t < count($select_contact_persons); $t++)
			                					{
								                    ...
								                    // $this->sma->send_email($select_all_director[$t]["email"], $subject, $message.'<p>Best regards,<br />Management on behalf of Acumen Alpha Advisory Group<br />Acumen Bizcorp<br />Address: 143 Cecil Street, #16-03 GB Building, Singapore 069542<br />Tel: (+65) 6220 1939</p>', 'admin@acumenbizcorp.com.sg', 'Acumen Secretary', null, 'looi@acumenbizcorp.com.sg, corpsec@acumenbizcorp.com.sg');
								                    array_push($fye_email_list, $select_contact_persons[$t]["email"]);
								                }
								                // $fye_unique_email = array_unique($fye_email_list);
								                // $email_detail['email'] = json_encode(array(array("email"=> $fye_unique_email[0])));


								                //check multiple email by special character
										        $fye_unique_email = array_unique($fye_email_list);
										        $email_to = explode(';', $fye_unique_email[0]); // your email address 
										        if(count($email_to) > 0)
										        {
										        	$email_arr = array();
										        ...
										        {
										        	$email_detail['email'] = json_encode(array(array("email"=> trim($fye_unique_email[0]))));
										        }
												//check multiple email by special character

								                //json_encode(array(array("email"=> "JUSTIN@AAA-GLOBAL.COM")));
								                //json_encode(array_unique($fye_email_list));
								                $email_detail['subject'] = $subject;
								                $email_detail['message'] = $message.'<p>Best regards,<br />Management on behalf of Acumen Alpha Advisory Group<br />ACUMEN ALPHA ADVISORY PTE. LTD.<br />Address: 18 Howard Road, #08-06 Novelty BizCentre, Singapore 369585<br />Tel: (+65) 62468801 / (+ 65) 62468802</p><table style="border: 1px solid black; border-collapse: collapse;"><tbody><tr style="border: 1px solid black;"><td style="border: 1px solid black;"><div style="font-size: 8px; font-family: Calibri, sans-serif; font-style: italic;text-align: justify;"><span>DISCLAIMER: This email and any files transmitted with it contain confidential information and is intended only for the individual named. If you are not the named addressee you should not disseminate, distribute or copy this e-mail. Please notify the sender immediately by e-mail if you have received this e-mail by mistake and delete this e-mail from your system. If you are not the intended recipient you are notified that disclosing, copying, distributing or taking any action in reliance on the contents of this information is strictly prohibited.</span><br/><br/><span>Information contained in this email is general in nature and does not take into account every aspect of your personal situation. You should consider the appropriateness of the information for your needs and if necessary seek professional advice.</span><br/><br/><span>WARNING: Computer viruses can be transmitted via email. The recipient should check this email and any attachments for the presence of viruses. Although the company has taken reasonable precautions to ensure no viruses are present in this email, the company cannot accept responsibility for any loss or damage arising from the use of this email or attachments.</span></div></td></tr></tbody></table>';
								                $email_detail['from_email'] = json_encode(array("name" => 'ACUMEN ALPHA ADVISORY SECRETARY', "email" => "admin@aaa-global.com"));//'admin@bizfiles.com.sg';
								                $email_detail['from_name'] = 'ACUMEN ALPHA ADVISORY SECRETARY';
								                $email_detail['attachment'] = null;
								                $email_detail['cc'] = json_encode(array(array("email" => "corpsec@aaa-global.com")));
								                //json_encode(array(array("email" => "then@acumenbizcorp.com.sg"), array("email" => "then.k.w@hotmail.com")));
								                //json_encode(array(array("email" => "corpsec@aaa-global.com")));
								                //json_encode(array(array("email" => "looi@aaa-global.com"), array("email" => "corpsec@aaa-global.com")));
								                $email_detail['bcc'] = null;
								                $email_detail['sended'] = 0;
								                $email_detail['type'] = 'year_end';
		                ...
		                if($filing_agm_info->num_rows() > 0)
		                {
						    ...
						    //$new_format_due_date_201 = date('Y-m-d', strtotime('1'.'-'.$month.'-'.(int)$year)); //change this date to manual send

						    //$new_format_due_date_201 = date('Y-m', strtotime($month.'-'.(int)$year));
						    $new_format_due_date_201 = date('Y-m-d', strtotime('1'.'-'.$month.'-'.(int)$year));
						    
		                	//if($new_format_due_date_201 == "1980-02") //take the filing agm
		                	if($new_format_due_date_201 == date('Y-m-d')) //take the filing agm
		                	{
		                		$client_info[$i]["company_name"] = $this->encryption->decrypt($client_info[$i]["company_name"]);
		                		$check_selected_reminder = $this->db->query("select document_reminder.*, client_setup_reminder.selected_reminder from client_setup_reminder left join document_reminder on document_reminder.id = client_setup_reminder.selected_reminder where client_setup_reminder.company_code='".$client_info[$i]["company_code"]."' and document_reminder.reminder_tag_id = 1 and document_reminder.active = 1");
		                		//echo json_encode($client_info[$i]["company_code"].", ");
		                		if ($check_selected_reminder->num_rows() > 0) 
			                	{	
			                		$check_selected_reminder = $check_selected_reminder->result_array();
				                		...
				                		if ($client_contact_info->num_rows() > 0) 
					                	{
							                ...
							                if ($select_contact_persons->num_rows() > 0) 
			                				{
			                					...
			                					for($t = 0; $t < count($select_contact_persons); $t++)
			                					{
								                    ...
								                    array_push($agm_email_list, htmlspecialchars($select_contact_persons[$t]["email"]));
								                }
								                // $agm_unique_email = array_unique($agm_email_list);
								                // $email_detail['email'] = json_encode(array(array("email"=> $agm_unique_email[0])));

								                //check multiple email by special character
										        $agm_unique_email = array_unique($agm_email_list);
										        $email_to = explode(';', $agm_unique_email[0]); // your email address 
										        if(count($email_to) > 0)
										        {
										        	$email_arr = array();
										        	for($t = 0; $t < count($email_to); $t++)
										        ...
										        {
										        	$email_detail['email'] = json_encode(array(array("email"=> trim($agm_unique_email[0]))));
										        }
												//check multiple email by special character

								                //json_encode(array(array("email"=> "JUSTIN@AAA-GLOBAL.COM")));
								                //json_encode(array_unique($agm_email_list));
								                $email_detail['subject'] = $subject;
								                $email_detail['message'] = $message.'<p>Best regards,<br />Management on behalf of Acumen Alpha Advisory Group<br />ACUMEN ALPHA ADVISORY PTE. LTD.<br />Address: 18 Howard Road, #08-06 Novelty BizCentre, Singapore 369585<br />Tel: (+65) 62468801 / (+ 65) 62468802</p><table style="border: 1px solid black; border-collapse: collapse;"><tbody><tr style="border: 1px solid black;"><td style="border: 1px solid black;"><div style="font-size: 8px; font-family: Calibri, sans-serif; font-style: italic; text-align: justify;"><span>DISCLAIMER: This email and any files transmitted with it contain confidential information and is intended only for the individual named. If you are not the named addressee you should not disseminate, distribute or copy this e-mail. Please notify the sender immediately by e-mail if you have received this e-mail by mistake and delete this e-mail from your system. If you are not the intended recipient you are notified that disclosing, copying, distributing or taking any action in reliance on the contents of this information is strictly prohibited.</span><br/><br/><span>Information contained in this email is general in nature and does not take into account every aspect of your personal situation. You should consider the appropriateness of the information for your needs and if necessary seek professional advice.</span><br/><br/><span>WARNING: Computer viruses can be transmitted via email. The recipient should check this email and any attachments for the presence of viruses. Although the company has taken reasonable precautions to ensure no viruses are present in this email, the company cannot accept responsibility for any loss or damage arising from the use of this email or attachments.</span></div></td></tr></tbody></table>';
								                $email_detail['from_email'] = json_encode(array("name" => 'ACUMEN ALPHA ADVISORY SECRETARY', "email" => "admin@aaa-global.com"));
								                $email_detail['from_name'] = 'ACUMEN ALPHA ADVISORY SECRETARY';
								                $email_detail['attachment'] = null;
								                $email_detail['cc'] = json_encode(array(array("email" => "corpsec@aaa-global.com")));
								                //json_encode(array(array("email" => "then@acumenbizcorp.com.sg"), array("email" => "then.k.w@hotmail.com")));
		     ...
		     //            $filing_eci_info = $this->db->query("select company_code, eci_tax_period, eci_filing_date, next_eci_filing_due_date from eci_filing where company_code='".$client_info[$i]["company_code"]."' order by filing.id DESC LIMIT 2");

		     //            if ($filing_eci_info->num_rows() > 0) 
		     //            {
		     //            	$filing_eci_info = $filing_eci_info->result_array();
		     //            	echo json_encode($filing_eci_info[0]);
					  //       $array = explode(' ', $filing_eci_info[0]["eci_tax_period"]);
					  //       $year = $array[2];
							// $month = $array[1];
							// $day = $array[0];

						 //    $new_format_eci_tax_period = date('Y-m-d', strtotime('5'.'-'.$month.'-'.(int)$year));

		     //            	if($new_format_eci_tax_period == date('Y-m-d')) //take the filing year end
		     //            	{
		     //            		$check_selected_reminder = $this->db->query("select document_reminder.*, client_setup_reminder.selected_reminder from client_setup_reminder left join document_reminder on document_reminder.id = client_setup_reminder.selected_reminder where client_setup_reminder.company_code='".$client_info[$i]["company_code"]."' and document_reminder.reminder_tag_id = 1 and document_reminder.active = 1");
		     //            		//echo json_encode($client_info[$i]["company_name"]);
		     //            		if ($check_selected_reminder->num_rows() > 0) 
	      ...
	      //   						{
			    ...
			    //             					{
							...
							// 	                    array_push($agm_email_list, $select_contact_persons[$t]["email"]);
							// 	                }

							// 	                $email_detail['email'] = json_encode(array_unique($agm_email_list));
							// 	                $email_detail['subject'] = $subject;
							// 	                $email_detail['message'] = $message.'<p>Best regards,<br />Management on behalf of Acumen Alpha Advisory Group<br />ACUMENBIZCORP PTE. LTD.<br />Address: 18 Howard Road, #08-06 Novelty BizCentre, Singapore 369585<br />Tel: (+65) 62468801 / (+ 65) 62468802</p><table style="border: 1px solid black; border-collapse: collapse;"><tbody><tr style="border: 1px solid black;"><td style="border: 1px solid black;"><div style="font-size: 8px; font-family: Calibri, sans-serif; font-style: italic; text-align: justify;"><span>DISCLAIMER: This email and any files transmitted with it contain confidential information and is intended only for the individual named. If you are not the named addressee you should not disseminate, distribute or copy this e-mail. Please notify the sender immediately by e-mail if you have received this e-mail by mistake and delete this e-mail from your system. If you are not the intended recipient you are notified that disclosing, copying, distributing or taking any action in reliance on the contents of this information is strictly prohibited.</span><br/><br/><span>Information contained in this email is general in nature and does not take into account every aspect of your personal situation. You should consider the appropriateness of the information for your needs and if necessary seek professional advice.</span><br/><br/><span>WARNING: Computer viruses can be transmitted via email. The recipient should check this email and any attachments for the presence of viruses. Although the company has taken reasonable precautions to ensure no viruses are present in this email, the company cannot accept responsibility for any loss or damage arising from the use of this email or attachments.</span></div></td></tr></tbody></table>';
							// 	                $email_detail['from_email'] = 'admin@aaa-global.com';
							// 	                $email_detail['from_name'] = 'Acumen Secretary';
							// 	                $email_detail['attachment'] = null;
							// 	                $email_detail['cc'] = 'looi@aaa-global.com, corpsec@aaa-global.com';
							// 	                $email_detail['bcc'] = null;
							// 	                $email_detail['sended'] = 0;
							// 	                $email_detail['type'] = 'agm';
							// 					$this->db->insert("email_queue",$email_detail);
   		...
   		if(count($match) != 0)
   		{
	   		for($r = 0; $r < count($match); $r++)
			{
				...
				elseif($string2 == "ECI date")
				{
					...
					$next_eci_filing_due_date = new DateTime($year_end_new[0]["year_end"]);
					...
					$content = $new_format_for_next_eci_due_date;
				...
				elseif($string2 == "AGM date")
				{
					...
					if($agm_date[0]["201_extended_to"] != '0')
					{
						$content = $agm_date[0]["201_extended_to"];
					}
					else
					{
						$content = $agm_date[0]["due_date_201"];
					}
				...
				elseif($string2 == "AR date")
				{
					...
					$agm_date = $this->db->query("select company_code, year_end, agm, due_date_175, 175_extended_to, due_date_201, 201_extended_to, due_date_197, 197_extended_to from filing where company_code='".$company_code."' order by filing.id DESC LIMIT 2");
					...
					// }
					if($agm_date[0]["197_extended_to"] != '0' && $agm_date[0]["197_extended_to"] != '')
					{
						$content = $agm_date[0]["197_extended_to"];
					}
					else
					{
				...
				elseif($string2 == "Provide financial statement date")
				{
					...
					if($agm_date[0]["201_extended_to"] != '0')
					{
						$due_date_175 = $agm_date[0]["201_extended_to"];
					}
					else
					{
						$due_date_175 = $agm_date[0]["due_date_201"];
					}
