...
class documents extends MY_Controller
{
    ...
    public function insert_pending_document_file() 
    {
        if($_POST["doc_type"] == 'trans')
        {
            ...
            $this->save_audit_trail("Document", "Add Document File", $company_name." document file is updated."); 
            ...
            if (count($this->session->userdata('pending_documents_files_id')) != 0)
            {
                ...
                {
                    $files = $this->db->query("select * from transaction_pending_documents_file where id='".$pending_documents_files_id[$i]."'");
                    ...
                    $this->db->delete('transaction_pending_documents_file', array('id' => $pending_documents_files_id[$i]));
                }
            }  
        ...
        if($type == "trans")
        {
            ...
            {   
                ...
                if($this->upload->do_upload('uploadpendingdocumentfile'))
                {
                    ...
                    chmod($_SERVER['DOCUMENT_ROOT'].$this->systemName.'/uploads/pending_document_file/'.$fileData['file_name'],0644);
                }
            }
    ...
    public function delete_transaction_document ()
    {
        ...
        $this->save_audit_trail("Document", "Index", $company_name." document is deleted."); 

        $this->db->delete("transaction_pending_documents",array('transaction_id'=>$transaction_pending_documents[0]["transaction_id"]));
    ...
    {
        $id = $_POST["master_id"];
        $master_name = $_POST["each_master_name"];
        $this->db->delete("document_master",array('id'=>$id));

        $this->save_audit_trail("Document", "Index", $master_name." master document is deleted."); 
    ...
    public function delete_reminder_document()
    {
        ...
        $reminder_name = $_POST["each_reminder_name"];
        $this->db->delete("document_reminder",array('id'=>$id));

        $this->save_audit_trail("Document", "Index", $reminder_name." reminder is deleted."); 
    ...
    public function add_document_toggle()
    {
        ...
        {   
            ...
            {               
                ...
                $this->save_audit_trail("Document", "Create Master", $_POST['document_name']." master document is added."); 
                /*$insert_client_charge_id = $this->db->insert_id();
                $this->create_invoice("change_charges", $_POST["company_code"]);*/
            } 
            else 
            {   
                ...
                $this->save_audit_trail("Document", "Create Master", $_POST['document_name']." master document is edited.");      
            }
            echo json_encode(array("Status" => 1, 'message' => 'Information Updated', 'title' => 'Updated'));
        }
    }
    ...
    public function add_document_reminder()
    {
        ...
        else
        {
            ...
            {               
                ...
                $this->save_audit_trail("Document", "Create Reminder", $_POST['reminder_name']." reminder is added."); 
            } 
            else 
            {   
                $this->db->update("document_reminder",$data,array("id" => $_POST['document_reminder_id']));

                $this->save_audit_trail("Document", "Create Reminder", $_POST['reminder_name']." reminder is edited.");      
            }
            echo json_encode(array("Status" => 1, 'message' => 'Information Updated', 'title' => 'Updated'));
        }
    }
