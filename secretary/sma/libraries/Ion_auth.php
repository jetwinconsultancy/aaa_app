...
class Ion_auth
{
    ...
    { //need to test email activation
        ...
        if (!$email_activation || $active == '1') {
            $id = $this->auth_model->register($username, $user_type_id, $password, $email, $additional_data, $term_and_condition, NULL, $active, $selected_firm, $manager_in_charge);
            if ($id !== FALSE) {
                if ($notify) {
                    $this->load->library('parser');
                    $parse_data = array(
                    ...
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $subject = 'New User Created - ' . $this->Settings->site_name;
                    $this->sma->send_email($email, $subject, $message);

                    $welcome_email_msg = file_get_contents('./themes/' . $this->theme . 'email_templates/welcome_email.html');
                    $welcome_email_message = $this->parser->parse_string($welcome_email_msg, $parse_data);
                    $welcome_email_subject = 'Welcome to our firm';
                    $this->sma->send_email($email, $welcome_email_subject, $welcome_email_message);
                }
