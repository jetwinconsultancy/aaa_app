...
class Auth_model extends CI_Model
{
    ...
    public function register($username, $user_type_id, $password, $email, $additional_data = array(), $term_and_condition,  $activation_code, $active = FALSE, $selected_firm = FALSE, $selected_person = FALSE)
    {
        ...
        if($additional_data["group_id"] != "4")
        {
            if (!isset($additional_data['group_id'])) {
                ...
                $additional_data['total_no_of_client'] = 10000;
            }
            else
            {
                $this->db->select('*');
        ...
        else
        {
            ...
            $user_person['user_admin_code_id'] = $this->session->userdata('user_admin_code_id');
            $user_person["user_id"] = $id;
            $user_person["officer_id"] = $selected_person;
            $this->db->insert('user_person', $user_person);
    ...
    public function token_endpoint($jwk,$authorization_code)
    {
        ...
        $jws = Build::jws() // We build a JWS
            ...
            ->sub('pjTydwFIkNTkEQntyHYwBIcxinFJwF75') //Testing Client ID: ANnWPio7eU2t8SYWHnmnh2Unpnd6Oqlh
            //->aud('https://stg-id.singpass.gov.sg')
            ->claim('aud', 'https://id.singpass.gov.sg')
            ->iss('pjTydwFIkNTkEQntyHYwBIcxinFJwF75') // The "iss" claim
        ...
        $data = ['client_id' => 'pjTydwFIkNTkEQntyHYwBIcxinFJwF75', 'redirect_uri'=>'https://bizfiles.com.sg/secretary/welcome', 'grant_type' => 'authorization_code', 'code' => $authorization_code, 'client_assertion_type' => 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer',  'client_assertion' => $jws];
    ...
    public function singpass_login($auth_code)
    {
        ...
        // set url
        curl_setopt($ch, CURLOPT_URL, "https://bizfiles.com.sg/secretary/jwks/getAAASingpassPrivatekey");
        ...
        // Our enc key
        $jwk = new JWK((array)$jwks_output->private_keys[0]);
        $ec_jwk = $jwks_output->private_keys[1];

        $result = $this->token_endpoint($jwk, $auth_code);

        $max_attempts = 1;

        // if($result->error == "invalid_client" && $max_attempts == 1)
        // {
        //     $max_attempts =+ 1;
        //     $cache_backup_key_storage = new FileStorage("./cache_backup_key");
        //     $cache_backup_key = new Cache($cache_backup_key_storage);
        //     $result_backup_key = $cache_backup_key->load($backup_key);

        //     if ($result_backup_key) {
        //         $total_object_number = count($result_backup_key);

        //         $jwk = $result_backup_key[$total_object_number - 1]->keys[0];
        //         $ec_jwk = $result_backup_key[$total_object_number - 1]->keys[1];

        //         $result = $this->token_endpoint($jwk, $authorization_code);

        //         if($result->error == "invalid_client" && $max_attempts == 2)
        //         {
        //             redirect('login');
        //         }
        //     }
        ...
        /* Array Parameter Data */
        $data = ['jwe' => $ec_token, 'jwt'=> [$ec_jwk], 'alg' => $jwtHeader->alg];
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select('username, email, id, password, active, last_login, last_ip_address, first_name, last_name, avatar, gender, group_id, date_of_expiry', 'user_deleted')
            ->where('identification_no', $this->db->escape_str($identification_no))
            ->where('user_deleted = 0')
            ->limit(1)
            ->get($this->tables['users']);

        if ($query->num_rows() === 1 && $identification_no != null) {
            $user = $query->row();

            if ($this->is_time_locked_out($user->email)) {
            ...
            else
            {
                ...
                $ldata = array('user_id' => $user->id, 'ip_address' => $this->input->ip_address(), 'login' => $user->email);
                ...
                return array("id" => $identification_no, "boolean" => true);
            }
        }
        ...
        return array("id" => $result, "boolean" => false);
    }
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            if($part == "access_client")
            {
                $this->db->select(array(
                    ...
                    'GROUP_CONCAT(user_client.client_id) AS client_id, user_person.officer_id'
                ));

                $this->db->join('user_client', 'user_client.user_id = '. $this->tables['users'] . '.id', 'left');
                $this->db->join('user_person', 'user_person.user_id = '. $this->tables['users'] . '.id', 'left');
            }
            else
            {
                $this->db->select(array(
