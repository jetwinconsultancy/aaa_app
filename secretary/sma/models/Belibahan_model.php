...
class Belibahan_model extends CI_Model
{
    ...
    public function getBahanMaster($term, $limit = 5)
    {
        $this->db->where("type = 'standard' AND reference_id = 0 AND (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        $this->db->limit($limit);
    ...
    {
        $this->db->where("(nama LIKE '%" . $term . "%' OR kode_bahan LIKE '%" . $term . "%' OR  concat(jenis, ' ',motif, ' ',warna) LIKE '%" . $term . "%')");
		$this->db->select("bahan_utama.*,bahan_master.bahan, bahan_master.id bahanid");
        $this->db->join('bahan_master','bahan_utama.kategori = bahan_master.id');
        $this->db->limit($limit);
        $q = $this->db->get('bahan_utama');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        if ($this->db->insert('bahan_u_beli', $data)) {
            $product_id = $this->db->insert_id();
            return $product_id;
        }
        return false;
	}
    public function addPurchase($data, $items,$input_ukuran)
    {           
        if ($this->db->insert('belibahan', $data)) {
            $purchase_id = $this->db->insert_id();
            if ($this->site->getReference('pb') == $data['reference_no']) {
                $this->site->updateReference('pb');
            }
            foreach ($input_ukuran as $i_u) {
                $i_u['purchase_id'] = $purchase_id;
                $this->db->insert('bahan_ukuran', $i_u);
			}
            foreach ($items as $item) {
                $item['belibahan_id'] = $purchase_id;
                $this->db->insert('belibahan_items', $item);
