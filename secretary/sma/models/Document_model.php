...
class Document_model extends CI_Model
{
    ...
    public function get_all_pending($type=NULL,$keyword=NULL,$start=NULL,$end=NULL){
        ...
        // $this->db->select('pending_documents.*, client.company_name, users.first_name, users.last_name, triggered_by.triggered_by as triggered_by_name');
        ...
        // $this->db->join('triggered_by', 'triggered_by.id = pending_documents.triggered_by', 'left');
        // if ($start != NULL)
        // {
        //     if ($end != NULL)
        //     {
        ...
        // $this->db->group_by(array('client_id', 'officer_id', 'controller_id', 'charge_id', 'filing_id', 'guarantee_id', 'allotment_id', 'buyback_id', 'transfer_id', 'triggered_by'));
        // $q = $this->db->get();
        ...
        //         if($keyword != null)
        //         {
        //             if(stripos($row->company_name, $keyword) !== FALSE)
        //             {
        //                 $data[] = $row;
        //             }
        //         }
        ...
        $this->db->select('transaction_pending_documents.*, transaction_client.company_name, users.first_name, users.last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
        ...
        $this->db->where('transaction_master.status != 4 and transaction_master.status != 5');
        $this->db->group_by(array('transaction_id', 'triggered_by'));
        $p = $this->db->get();
        ...
        if ($p->num_rows() > 0) {
            foreach (($p->result()) as $row) {
                if($row->received_on == "")
                {
                    ...
                    if($keyword != null)
                    {
                        if(mb_stripos($row->company_name, $keyword) !== FALSE)
                        {
                            $data[] = $row;
                        }
                    }
        ...
        $this->db->select('transaction_pending_documents.*, transaction_master.client_name as company_name, users.first_name, users.last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
        $this->db->from('transaction_pending_documents');
        //$this->db->join('transaction_client', 'transaction_client.id = transaction_pending_documents.client_id', 'left');
        $this->db->join('users', 'users.id = transaction_pending_documents.created_by', 'left');
        $this->db->join('transaction_master', 'transaction_master.id = transaction_pending_documents.transaction_id', 'left');
        ...
        //     else 
        //     {
                //$this->db->where("(transaction_pending_documents.document_name LIKE '%".$keyword."%')", NULL, FALSE);
        ...
        $this->db->select('transaction_pending_documents.*, client.company_name, users.first_name, users.last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
        $this->db->from('transaction_pending_documents');
        $this->db->join('users', 'users.id = transaction_pending_documents.created_by', 'left');
        $this->db->join('transaction_master', 'transaction_master.id = transaction_pending_documents.transaction_id', 'left');
    ...
    public function get_all_document($group_id=NULL,$type=NULL,$keyword=NULL,$start=NULL,$end=NULL,$transaction_master_id=NULL)
    {
        ...
        // }
        // $this->db->where("(pending_documents.document_name LIKE '%".$keyword."%')", NULL, FALSE);
        ...
        if($group_id != 4)
        {
            $this->db->select('transaction_pending_documents.*, transaction_client.company_name, created_by_user.first_name as created_by_first_name, created_by_user.last_name as created_by_last_name, received_by_user.first_name as received_by_first_name, received_by_user.last_name as received_by_last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
            $this->db->from('transaction_pending_documents');
            //$this->db->join('transaction_client', 'transaction_client.id = transaction_pending_documents.client_id', 'left');
            ...
            $this->db->select('transaction_pending_documents.*, transaction_master.client_name as company_name, created_by_user.first_name as created_by_first_name, created_by_user.last_name as created_by_last_name, received_by_user.first_name as received_by_first_name, received_by_user.last_name as received_by_last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
            $this->db->from('transaction_pending_documents');
            ...
            $this->db->where('transaction_pending_documents.firm_id', $this->session->userdata("firm_id"));
            $this->db->where('(transaction_master.transaction_task_id = 29 or transaction_master.transaction_task_id = 30 or transaction_master.transaction_task_id = 35)');
            //$this->db->where("(transaction_pending_documents.document_name LIKE '%".$keyword."%')", NULL, FALSE);
            $this->db->where('transaction_master.status != 4 and transaction_master.status != 5');
            // if ($type != NULL)
            // {
        ...
        $this->db->select('transaction_pending_documents.*, client.company_name, created_by_user.first_name as created_by_first_name, created_by_user.last_name as created_by_last_name, received_by_user.first_name as received_by_first_name, received_by_user.last_name as received_by_last_name, transaction_tasks.transaction_task as triggered_by_name, transaction_master.transaction_code');
        $this->db->from('transaction_pending_documents');
        // $this->db->join('client', 'client.id = transaction_pending_documents.client_id', 'left');
        $this->db->join('users as created_by_user', 'created_by_user.id = transaction_pending_documents.created_by', 'left');
        $this->db->join('users as received_by_user', 'received_by_user.id = transaction_pending_documents.received_by', 'left');
        ...
        $this->db->where('transaction_pending_documents.firm_id', $this->session->userdata("firm_id"));
        $this->db->where('transaction_master.transaction_task_id != 1 and transaction_master.transaction_task_id != 29 and transaction_master.transaction_task_id != 30 and transaction_master.transaction_task_id != 35');
        $this->db->where('transaction_master.status != 4 and transaction_master.status != 5');
        $this->db->group_by(array('transaction_id', 'triggered_by'));
        $q = $this->db->get();
    ...
    public function get_pending_document($id, $type = null){
        ...
        if($type != null)
        {
            $q = $this->db->query("select transaction_pending_documents.*, client.company_name, users.first_name, users.last_name, transaction_master.client_name from transaction_pending_documents left join users on users.id = transaction_pending_documents.created_by left join transaction_master on transaction_master.id = transaction_pending_documents.transaction_id left join client on client.company_code = transaction_master.company_code where transaction_pending_documents.firm_id='".$this->session->userdata('firm_id')."' AND transaction_pending_documents.id = '".$id."' AND transaction_master.registration_no != ''");
            //print_r($q->result_array());
            if (0 == $q->num_rows()) {
                $q = $this->db->query("select transaction_pending_documents.*, transaction_client.company_name, users.first_name, users.last_name, transaction_master.client_name from transaction_pending_documents left join users on users.id = transaction_pending_documents.created_by left join transaction_master on transaction_master.id = transaction_pending_documents.transaction_id left join transaction_client on transaction_client.company_code = transaction_master.company_code where transaction_pending_documents.firm_id='".$this->session->userdata('firm_id')."' AND transaction_pending_documents.id = '".$id."'");
            }
        }
        else
        {
    ...
    public function get_pending_document_file($id, $type = null){
        ...
        if($type != null)
        {
            ...
            $q = $this->db->query("select transaction_pending_documents.*, client.company_name, transaction_master.client_name, GROUP_CONCAT(DISTINCT CONCAT(transaction_pending_documents_file.id,',', transaction_pending_documents_file.file_name)SEPARATOR ';') AS 'company_files' from transaction_pending_documents LEFT JOIN transaction_pending_documents_file ON transaction_pending_documents_file.pending_documents_id = transaction_pending_documents.id left join transaction_master on transaction_master.id = transaction_pending_documents.transaction_id left join client on client.company_code = transaction_master.company_code where transaction_pending_documents.id='".$id."' AND transaction_master.registration_no != '' AND transaction_master.registration_no IS NOT NULL");

            //print_r($q->result_array());

            if (0 == $q->num_rows() || $q->result()[0]->id == "") {
                $q = $this->db->query("select transaction_pending_documents.*, transaction_client.company_name, transaction_master.client_name, GROUP_CONCAT(DISTINCT CONCAT(transaction_pending_documents_file.id,',', transaction_pending_documents_file.file_name)SEPARATOR ';') AS 'company_files' from transaction_pending_documents LEFT JOIN transaction_pending_documents_file ON transaction_pending_documents_file.pending_documents_id = transaction_pending_documents.id left join transaction_master on transaction_master.id = transaction_pending_documents.transaction_id left join transaction_client on transaction_client.company_code = transaction_master.company_code where transaction_pending_documents.id='".$id."'"); //  AND transaction_master.registration_no = '' GROUP BY transaction_pending_documents.id
                //print_r($q->result_array());
            }
        }
        else
