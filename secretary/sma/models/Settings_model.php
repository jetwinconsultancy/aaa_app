...
class Settings_model extends CI_Model
{
    ...
    {
        //$q = $this->db->get_where('email_queue');
        $this->db->where('MONTH(created_at) = MONTH(CURDATE())');
        $this->db->where('YEAR(created_at) = YEAR(CURDATE())');
        $q = $this->db->get('email_queue');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->select('secretary_audit_trail.*, users.first_name, users.last_name');
        $this->db->from('secretary_audit_trail');
        $this->db->join('users', 'users.id = secretary_audit_trail.user_id', 'left');
        $this->db->order_by("secretary_audit_trail.id", "desc");
        $q = $this->db->get();
