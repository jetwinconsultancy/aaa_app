...
class Db_model extends CI_Model
{
	...
	public function getClient($tipe=NULL,$keyword=NULL)
    {
        // if ($this->Settings->restrict_user && !$this->Owner && !$this->Admin) {
            // $this->db->where('created_by', $this->session->userdata('user_id'));
        // }
		...
		if ($tipe != NULL)
		{
			if ($tipe != 'All')
			{
				...
				$this->db->group_start();
				$this->db->or_like('clientcode', $keyword);
				$this->db->or_like('uen', $keyword);
				$this->db->or_like('date_incorporation', $keyword);
				$this->db->or_like('client_name', $keyword);
				$this->db->or_like('postal_code', $keyword);
				$this->db->or_like('city', $keyword);
				$this->db->or_like('streetname', $keyword);
				$this->db->or_like('buildingname', $keyword);
				$this->db->or_like('unitno', $keyword);
				$this->db->or_like('unitno1', $keyword);
				...
				$this->db->or_like('chairman', $keyword);
				$this->db->or_like('formername', $keyword);
				$this->db->group_end();
			}
		}
        $this->db->order_by('id', 'desc');
        $q = $this->db->get_where("client");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
