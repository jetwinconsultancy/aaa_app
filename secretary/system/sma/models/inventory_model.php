...
class Inventory_model extends CI_Model
{
    ...
    public function addHasil($items = array())
    {
        if ($this->db->insert('pola', $items)) {
            $pola_id = $this->db->insert_id();
			return $pola_id;
        }
        return false;
    }
    public function addHeaderPola($items = array())
    {
        if ($this->db->insert('pola_utama', $items)) {
            $pola_utama_id = $this->db->insert_id();
			return $pola_utama_id;
        }
        return false;
    }
    public function addPolaTambahan($items = array())
    {
        if ($this->db->insert('pola_tmbh', $items)) {
            $pola_tambahan = $this->db->insert_id();
			return $pola_tambahan;
        }
        return false;
    }
    public function addPolaStockBahan($items = array())
    {
        if ($this->db->insert('pola_stock_bahan', $items)) {
            $pola_stock_bahan = $this->db->insert_id();
			return $pola_stock_bahan;
        }
        return false;
    }
    public function addPolaBahanTambahan($items = array())
    {
        if ($this->db->insert('pola_bahan_tambahan', $items)) {
            $pola_stock_bahan = $this->db->insert_id();
			return $pola_stock_bahan;
        }
        return false;
