...
class Produksi_model extends CI_Model
{
    ...
    public function addRepacking($data = array(), $items = array())
    {
        if ($this->db->insert('repacking', $data)) {
            $repacking_id = $this->db->insert_id();
            if ($this->site->getReference('rep') == $data['reference_no']) {
                $this->site->updateReference('rep');
            }
            foreach ($items as $item) {

                $item['repacking_id'] = $repacking_id;
                $this->db->insert('repacking_items', $item);
                $repacking_item_id = $this->db->insert_id();
                if ($data['repacking_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {
					$this->sync_repackingQTY($item['product_id'],$item['warehouse_id'],$item['quantity'],$item['raw_fg']);
                    // $item_costs = $this->site->item_costing($item);
                    // foreach ($item_costs as $item_cost) {
                        // $item_cost['repacking_item_id'] = $repacking_item_id;
	...
	public function addTransfer($data=array() ,$items = array())
    {
		if ($this->db->insert('trans', $data)) {
            ...
            if ($this->site->getReference('tts') == $data['reference_no']) {
                $this->site->updateReference('tts');
            }
            foreach ($items as $item) {

                $item['id_trans'] = $trans_id;
				$this->db->insert('trans_item', $item);
            }
			return $trans_id;
	...
	public function sync_repackingQTY($product_id, $warehouse_id,$quantity,$raw_fg) 
	{
		if ($raw_fg == 1)
		{
			...
			if ($this->db->update('products')) {
				if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
					$this->db->update('warehouses_products', array('quantity' => 'quantity-'.$quantity), array('product_id' => $product_id, 'warehouse_id' => $warehouse_id));
				} else {
					if( ! $wh_balance_qty) { $wh_balance_qty = 0; }
					$this->db->insert('warehouses_products', array('quantity' => '0-'.$quantity, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id));
				}
				return TRUE;
			}
		} else if ($raw_fg == 2)
		{
			...
			if ($this->db->update('products')) {
				if ($this->getWarehouseProducts($product_id, $warehouse_id)) {
					...
					if( ! $wh_balance_qty) { $wh_balance_qty = 0; }
					$this->db->insert('warehouses_products', array('quantity' => $quantity, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id));
				}
				return TRUE;
			}
		}
    ...
    public function getRepackingByID($id)
    {
        $q = $this->db->get_where('repacking', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    {
        $q = $this->db->get_where('trans', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    }
	public function getTransferR($id)
    {
        $q = $this->db->get_where('trans', array('keid' => $id, 'status' => 1));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function getTransferRAll($id)
    {
		$this->db->select("trans_item.*,products.name")
			->join('products','products.id=trans_item.product_id');
        $q = $this->db->get_where('trans_item', array('keid' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
		$this->db->select("distinct `date`, dari, ke")
            ->group_by('`date`')
            ->group_by('dari')
            ->group_by('ke')
            ->order_by('id', 'desc');
        $q = $this->db->get_where('trans');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function deleteRepacking($id)
    {
        $repacking_item = $this->resetRepackingActions($id);
        if ($this->db->delete('repacking_items', array('repacking_id' => $id)) &&
        $this->db->delete('repacking', array('id' => $id))) {
    ...
    public function resetRepackingActions($id)
    {
		// print_r($id);
        $items = $this->getAllRepackingItems($id);
		// print_r($items);
        foreach ($items as $item) {
			$this->sync_repackingQTY($item->product_id,$item->warehouse_id,(0-$item->quantity),$item->raw_fg);
    ...
    public function getAllRepackingItems($repacking_id)
    {
        $this->db->select('raw_fg,product_id,product_code,product_name,product_type,unit_price,quantity')
            ->group_by('repacking_items.id')
            ->order_by('id', 'asc')
            ->order_by('raw_fg', 'asc');
        $q = $this->db->get_where('repacking_items', array('repacking_id' => $repacking_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function calculate_repacking()
	{
		/*----Update QTY Barang-----*/
	...
	public function getProductNames1($term, $warehouse_id, $limit = 5)
    {
        $this->db->select('products.id, code, name, type, toko_products.quantity,unit,unit1,unit_c1,unit2,unit_c2,unit_primary, price, tax_rate, tax_method, products.cost')
            ->join('toko_products', 'toko_products.product_id=products.id', 'left')
            ->group_by('products.id');
        if ($this->Settings->overselling) {
            $this->db->where("(name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        } else {
            $this->db->where("(products.track_quantity = 0 OR toko_products.quantity > 0) AND toko_products.warehouse_id = '" . $warehouse_id . "' AND "
                . "(name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        }
        $this->db->limit($limit);
        $q = $this->db->get('products');
	...
	public function Opname_updateAdjustment($id)
	{
		$q = $this->db->query('select date,quantity,note from sma_adjustments where product_id='.$id.' order by date asc');
		if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
