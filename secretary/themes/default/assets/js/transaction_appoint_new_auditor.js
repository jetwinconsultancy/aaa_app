...
function change_auditor_reason_selection(element){
	var tr = jQuery(element).parent().parent();

	var selected = tr.find(".resign_auditor_reason_selection").val();
	tr.find("#hidden_resign_auditor_reason_selection").val(selected);

	if(selected == 'OTHERS')
	{
		tr.find(".resign_auditor_reason").show();
	}
	else
	{
		tr.find(".resign_auditor_reason").hide();
...
function withdraw_auditor(element)
{
	...
	if(jQuery(element).html() == 'Withdraw')
	{
		...
		tr.find(".resign_auditor_reason_selection").prop("disabled", false);
	...
	else
	{
		...
		tr.find("#resign_auditor_reason").hide();
		tr.find(".resign_auditor_reason_selection").prop("disabled", true);

		tr.find(".officer_date_of_cessation").val("");
		tr.find(".resign_auditor_reason_selection").val("");
		tr.find("#hidden_resign_auditor_reason_selection").val("null");
...
$(document).on('click',"#submitAuditorInfo",function(e){
    ...
    $.ajax({ 
      ...
      type: "POST",
      data: $('form#apointment_of_auditor_form').serialize()+ '&transaction_code=' + $('#transaction_code').val() + '&transaction_task_id=' + $('#transaction_task').val() + '&registration_no=' + $('#uen').val()+ '&transaction_master_id=' + $("#transaction_trans #transaction_master_id").val(),
      dataType: 'json',
