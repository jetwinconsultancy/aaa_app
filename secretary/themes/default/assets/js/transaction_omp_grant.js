$(document).on('click',"#submitOMPGrantInfo",function(e){
    $('#loadingmessage').show();
    ...
    $.ajax({ //Upload common input
      url: "transaction/add_omp_grant",
      type: "POST",
      data: $('form#omp_grant_form').serialize()+ '&transaction_code=' + $('#transaction_code').val() + '&transaction_task_id=' + $('#transaction_task').val() + '&client_type=' + $('.client_type').val() + '&client_name=' + encodeURIComponent(tran_company_name) + '&transaction_master_id=' + $("#transaction_trans #transaction_master_id").val(),
      dataType: 'json',
