...
<script>
    ...
    var trans_date = null, affect_officer_id = null, affect_field_type = null;
            ...
            if (result) {
               $('#loadingmessage').show();
                $.ajax({ //Upload common input
                        url: "masterclient/delete_subsequent_transfer",
                        type: "POST",
    ...
    if(member_info.length > 0)
    {
        ...
        {
            for(var y = 0; y < certificate_no.length; y++)
            {
                if(certificate_no[y] == member_info[i]["certificate_no"])
                {
                    //console.log(member_info[i]["trans_date"]);
                    //console.log(member_info[i]["officer_id"]);
                    trans_date = member_info[i]["trans_date"];
                    affect_officer_id = member_info[i]["officer_id"];
                    affect_field_type = member_info[i]["field_type"];
                }
            }
            ...
            $b=""; 
            if(member_info[i]["trans_date"] > trans_date && trans_date != null && member_info[i]["officer_id"] == affect_officer_id && member_info[i]["field_type"] == affect_field_type)
            {
