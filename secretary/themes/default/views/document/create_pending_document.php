		...
		<div class="col-md-12">
			...
			<section class="panel">
				...
				<div class="panel-body">
					...
					?>
					<input type="hidden" name="pending_document_id" class="form-control" id="pending_document_id" value="<?=isset($document_id)?$document_id:'' ?>"/>
					<table class="table table-bordered table-striped table-condensed mb-none">
						<tr>
							<th>Client Name</th>
							<td>
								<div class="document-input-group" style="width: 100%;">
									<div class="validate" style="width: 50%;"><input type="text" name="client_name" class="form-control client_name" id="client_name" value="<?=isset($this->data['pending_document'])?(($this->data['pending_document'][0]->client_name != null)?$this->data['pending_document'][0]->client_name:$this->data['pending_document'][0]->company_name):''?>"/><input type="hidden" name="client_id" class="form-control" id="client_id" value="<?=isset($pending_document[0]->client_id)?$pending_document[0]->client_id:''?>"/></div>
						        </div>
						...
						<tr>
							...
							<td>
								<div class="document-input-group">
									<div class="validate" style="width: 50%;"><input type="text" name="document_name" class="form-control document_name" id="document_name" value="<?=isset($pending_document[0]->document_name)? $pending_document[0]->document_name : ''?>"/></div>
								</div>
							...
							<td>
								<div style="width:400px">
								    <div style="margin-right:5px;float: left">
								        <input type="checkbox" name="document_date_checkbox" <?=isset($pending_document[0]->document_date_checkbox)?'checked':'';?>/>
								    ...
								    <div>
								        <div class="document-input-group" id="document_transaction_date" style="width: 220px;position: relative;display: table;border-collapse: separate;">
											...
											<input type="text" class="document_transaction_date form-control" name="document_transaction_date" data-date-format="dd/mm/yyyy" data-plugin-datepicker required value="<?=isset($pending_document[0]->transaction_date)?$pending_document[0]->transaction_date:''?>" style="border-bottom-left-radius: 0;border-top-left-radius: 0;">
										</div>
					...
					<br/>
					<div class="document-input-group">
						<textarea id="pending_document_tinymce" class="tinymce" name="pending_document_content"><?=isset($pending_document[0]->content)?$pending_document[0]->content:''?></textarea>
...
<script>
	var pending_document = <?php echo json_encode(isset($pending_document)?$pending_document:'') ?>;
